...
-- -- VERSÃO PARA UNICO REGISTRO NA TABELA DO BANCO DE DADOS
-- CREATE TABLE IF NOT EXISTS consumo (
--     id INT AUTO_INCREMENT PRIMARY KEY,              -- Identificador único do consumo
--     reserva INT NOT NULL,                           -- Identificador da reserva associada ao consumo
--     funcionario INT NOT NULL,                       -- Identificador do funcionário que registrou o consumo
--     itens JSON NOT NULL,                            -- Armazenamento dos itens consumidos em formato JSON
--     totalDeItens INT NOT NULL,                     -- Quantidade total de itens consumidos
--     FOREIGN KEY (reserva) REFERENCES reserva(id),  -- Relaciona com a tabela reserva
--     FOREIGN KEY (funcionario) REFERENCES funcionario(id)  -- Relaciona com a tabela funcionario
-- ) ENGINE=InnoDB;
...
--     (c.quantidade * i.preco) AS total_item,
--     SUM(c.quantidade * i.preco) OVER (PARTITION BY r.id) AS total_reserva
-- FROM
--     consumo c
-- JOIN
--     item i ON c.item = i.id
-- JOIN
--     reserva r ON c.reserva = r.id
-- WHERE
--     r.id = 1  -- Substitua '1' pelo ID da reserva que você quer consultar
